...
;(function(){
  ...
  window.app = {
    ...
    //购物袋部分
    ajaxWithLoadMorebag: function(params) {
      var _params
      var fn = function(){
          if(fn.status !== 'NOMORE') {
            app.ajax(_params)
      ...
      fn.init = function(){
        ...
        _params.data.pageSize = 10
        _params.data.pageNum  = 1
        $('.load-mores').html('<span style="background:url(../../image/loading.gif) no-repeat; width:22px;height:22px;background-size:22px;vertical-align:middle;display:inline-block;margin-right:5px;"></span>加载更多...')
        if(params.success) {
          _params.success = function(rst){
            params.success(rst)
            _params.data.pageNum++
            console.log('rst.data.length='+rst.data.length)
            console.log('_params.data.pageSize='+_params.data.pageSize)
            if(rst.length <_params.data.pageSize) {
              console.log('---------------------------------')
              fn.status = "NOMORE"
   ...
   ajaxWithLoadMorePush: function(params) {
    ...
    dataNull: function(container, cls){
        $(container).html('<div class="data-null ' + (cls ? cls : '') + '">您的购物车暂无商品 赶紧去购物吧</div>')
    },
    // 页面模板
    template: {
