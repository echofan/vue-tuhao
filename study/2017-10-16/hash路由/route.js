/*
    1:注册路由：spaRoutes.map('/name',function(transition){
                    //异步加载js
                    spaRoutes.asyncFun('name.js',transition)
                    //或者同步执行回调
                    spaRoutes.syncFun(function(transition){},transition)
               })
    2:初始化：spaRoutes,init()
    3:跳转  href='#/name'            
*/
...
(function(){
    ...
    spaRouters.prototype={
        ...
        map:function(path,callback){
            ...
            if(callback&&Object.prototype.toString.call(callback)==='[object Function]'){  //存在回调，且回调是一个正确的函数的情况下进行存储  以/name  为key的对象
                this.routers[path]={
                    callback:callback,  //回调
                    fn:null
                }  
            }else{
                console.log('注册'+path+'地址需要注册正确的地址回报')
            }
